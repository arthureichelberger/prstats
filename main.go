...
func main() {
	...
	log.Debug().Str("prID", prID).Msg("fetched PR ID")

	deleteurl := fmt.Sprintf("https://api.github.com/repos/%s/pulls/comments/%s", repository, "1039433778")
	deletereq, _ := http.NewRequest(http.MethodDelete, deleteurl, http.NoBody)
	deletereq.Header.Add("Authorization", fmt.Sprintf("token %s", secret))
	call(deletereq)
